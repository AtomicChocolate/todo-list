...
document
  ...
  .addEventListener("click", UI.expandCollapseSidebar);
...
function projectNameDefocus() {
  const currentProject = Storage.findProject(
    selectedProject.querySelector("span").textContent
  );

  if (projectName.value !== "") {
    const newProjectVersion = new Project(
      projectName.value,
...
function taskComplete(event, taskDiv, input) {
  ...
  const currentTask = Storage.findTask(
    currentProject,
...
function newTaskAdd() {
  ...
  if (newTaskName.value === "") {
    ...
    Storage.findTask(currentProject, newTaskName.value) !== undefined
  ) {
    newTaskName.value = "";
